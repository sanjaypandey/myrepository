/*
 * employee_with_mutex.cpp
 * This file insures the Employee of the day by using the mutex on it.
...
void* do_loop(void* data)
{
    ...
    while (1) {
        ...
        if(my_num==1)
              printf("\nthread 1..copied the data\n");
         else
              printf("\nthread 2..copied the data\n");

    }
    return NULL;
}

/* like any C program, program's execution begins in Driver */
void driver()
{
		int        i;              /* loop counter                          */
        ...
        if(thr_id1==0)
        	 printf("\nthread 1 is creadted successfully..\n");
        else printf("\nthread 1 is not creadted ..\n");
	    /* create a second thread that will execute 'do_loop()' with '2'    */
	    thr_id2 = pthread_create(&p_thread2, NULL, do_loop, (void*)&num2);
	    if(thr_id2==0)
	          	 printf("\nthread 2 is creadted successfully..\n");
	    else printf("\nthread 2 is not creadted ..\n");
