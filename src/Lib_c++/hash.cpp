...
bool hash_init (struct hash *h,hash_hash_func *hash, hash_less_func *less, void *aux)
{
  h->elem_cnt = 0;
  h->bucket_cnt = 4;
  h->buckets =(struct list *) malloc(sizeof*h->buckets*h->bucket_cnt);
  h->hash = hash;
  h->less = less;
  h->aux = aux;
  if (h->buckets != NULL)
...
struct hash_elem *hash_insert (struct hash *h, struct hash_elem *newh)
{
  struct list *bucket = find_bucket (h,newh);
  struct hash_elem *old = find_elem (h, bucket, newh);

  if (old == NULL)
    insert_elem (h, bucket, newh);
...
struct hash_elem *hash_replace (struct hash *h, struct hash_elem *newh)
{
  struct list *bucket = find_bucket (h, newh);
  struct hash_elem *old = find_elem (h, bucket, newh);
